...
export default class ProductDetailTwo {
 ...
 public availableColors = computed(() => {
  ...
  const currentIndex = imgs.findIndex(img => img.src === this.activeImage());
  // if (currentIndex < imgs.length - 1) {
  //   this.activeImage.set(imgs[currentIndex + 1].src);
  // } else {
  //   this.activeImage.set(imgs[0].src); // Vuelve a la primera
  // }
}
...
prevImage() {
  ...
  //   this.activeImage.set(imgs[currentIndex - 1].src);
  // } else {
  //   this.activeImage.set(imgs[imgs.length - 1].src); // Va a la última
  // }
}
